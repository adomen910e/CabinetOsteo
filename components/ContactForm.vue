<template>
  <div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-2xl font-bold mb-6 text-center">Contact</h2>
    <form @submit.prevent="sendEmail" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="mb-4">
        ...
        <select v-model="animal" required
          class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-500">
      ...
      <button type="submit"
        class="w-full bg-bleu text-white font-semibold py-2 rounded-md hover:bg-bleu_hover transition duration-200 col-span-1 md:col-span-2">Envoyer</button>
    </form>
  ...
  methods: {
    sendEmail() {
      const templateParams = {
        ...
        phone: this.phone,
        user_email: this.email, // Utilisez l'email de la personne soumettant le formulaire
